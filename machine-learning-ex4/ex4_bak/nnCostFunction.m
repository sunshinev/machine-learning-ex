...
for i = 1:m
	for k = 1:num_labels
		...
		J_k(i,k) = y_k*log(h_k)+(1-y_k)*log(1-h_k);
...
% size(Theta1_NoBias) 25 400
Theta2_NoBias = Theta2(:,2:(size(Theta2,2)));
% size(Theta2_NoBias) 10 25

L = 3;

% for l = 1:(L - 1)
	lambdaValue = sum((Theta1_NoBias.^2)(:)) + sum((Theta2_NoBias.^2)(:));
% end

lambdaValue

J = J+lambda*lambdaValue/(2*m);
...
% 将每层，每个单元的误差进行求和，就得到
Delta_1 = Delta_1 + delta_2'*X; % 求导 链式法则 得到的是theta1 的误差梯度值
